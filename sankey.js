            ...
            d3.json("callouts_structure.json").then(function (graph) {
             ...
             // Process each interruption to count calls made and received and refer to the names
             graph.interruptions.forEach(d => {
                 if (totalCalls[d.caller]) {
                     totalCalls[d.caller]++;
                 } else {
                     totalCalls[d.caller] = 1;
                 }
                 if (d.speaker) {
                     if (receivedCalls[d.speaker]) {
                         receivedCalls[d.speaker]++;
                     } else {
                         receivedCalls[d.speaker] = 1;
                     }
                 }
             });
